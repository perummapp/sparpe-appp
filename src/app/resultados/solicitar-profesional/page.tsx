...
export default function SolicitarProfesionalPage() {
  ...
  const handleEnviar = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('solicitudes_profesional').insert({
  ...
  if (solicitudExistente) {
    const textoEstado: Record<string, string> = {
      ...
      aprobado: '¡Tu solicitud fue aprobada! Ya deberías tener acceso a "Mis resultados".',
    ...
    return (
      <div className="min-h-screen bg-[#0d0d0d] flex items-center justify-center p-6">
        <div className="w-full max-w-sm bg-[#161616] border border-[#262626] rounded-2xl p-8 text-center">
          <h1 className="text-xl font-bold text-white mb-3">Registro de peleador profesional</h1>
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] flex items-center justify-center p-6">
      <div className="w-full max-w-sm bg-[#161616] border border-[#262626] rounded-2xl p-8">
        <h1 className="text-2xl font-bold text-white mb-2">Registra tu trayectoria profesional</h1>
        <p className="text-sm text-[#9a9a9a] mb-6">
          "Mis resultados" es un historial oficial de peleas reales (no sparrings). Revisamos cada solicitud antes de habilitar el acceso.
        </p>
        <form onSubmit={handleEnviar}>
          <label className={labelClass}>Disciplina principal</label>
          <input type="text" value={disciplina} onChange={(e) => setDisciplina(e.target.value)} required placeholder="Ej. MMA" className={inputClass} />

          <label className={labelClass}>Nivel / trayectoria</label>
          <textarea value={nivelExperiencia} onChange={(e) => setNivelExperiencia(e.target.value)} rows={2} placeholder="Ej. Amateur federado, 8 peleas registradas" className={inputClass} />

          <label className={labelClass}>Evidencia (opcional)</label>
          <input type="text" value={evidencia} onChange={(e) => setEvidencia(e.target.value)} placeholder="Link a Tapology, video, escuela que respalda" className={inputClass} />
